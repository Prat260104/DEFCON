...
export const RULES: RiskRule[] = [
  ...
  { pattern: /rm\s+(-[a-zA-Z]*r[a-zA-Z]*f|--recursive|--force)\b/, level: "high", label: "recursive delete" },
  ...
  { pattern: /^(npm|yarn|pnpm)\s+(install|add|remove|uninstall)\b/, level: "medium", label: "dependency change" },
  ...
  { pattern: /^git\s+(status|log|diff|branch|show|tag|stash list)\b/, level: "low", label: "git read" },
  { pattern: /^(npm|yarn|pnpm)\s+(test|run\s+test)\b/, level: "low", label: "test run" },
  { pattern: /^(ls|cat|pwd|echo|head|tail|wc|file|which|whoami|date|uname)\b/, level: "low", label: "read-only shell" },
  { pattern: /^(npm|yarn|pnpm)\s+run\s+(lint|build|typecheck|check|format)\b/, level: "low", label: "build/lint" },
